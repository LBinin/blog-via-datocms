...
const EditorView: React.FC<{
  ...
  return (
    <div className={`${EditorClass.TabActiveBg} rounded-lg border ${EditorClass.Border} overflow-hidden relative group`}>
      {/* 标题区域 */}
      {!simpleMode && (
        <div className={`mt-2 flex ${EditorClass.Text} text-xs sm:text-sm border-b ${EditorClass.Border}`}>
          <div className="flex overflow-auto scroll-m-0 scrollbar-hide">
            {codeBlockList.map((item, index) => (
              <button
                ...
                className={classnames(
                  `px-3 sm:px-5 py-2 border-t whitespace-nowrap ${EditorClass.BorderLight} cursor-pointer`,
                  // 未激活样式
                  { [`${EditorClass.TabBg}`]: index !== currTab },
                  { 'border-r': index !== currTab && index !== codeBlockList.length - 1},
                  // 激活时的样式
          ...
          {/* 右侧功能区 */}
          <div className={classnames(`${EditorClass.TabBg} border-t border-l ${EditorClass.BorderLight} flex-1 flex justify-end pr-2 sm:pr-3 space-x-0 sm:space-x-2`, { 'rounded-tl': currTab === codeBlockList.length - 1 })}>
            {functionalOperation}
          </div>
        </div>
      )}
      ...
      {simpleMode && (
        <div className={`absolute overflow-auto right-2 top-2 flex transition opacity-0 group-hover:opacity-100 ${EditorClass.Text} space-x-1 sm:space-x-2`}>
          {functionalOperation}
        </div>
      )}
