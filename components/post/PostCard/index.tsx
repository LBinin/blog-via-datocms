  ...
  return (
    <Link href={`/posts/${post.slug}`}>
      <a
        ...
        className="group mx-5 my-10 flex flex-col items-stretch overflow-hidden rounded-md border transition duration-300 hover:shadow-xl md:mx-0 md:flex-row"
      >
        <div className="relative w-full shrink-0 md:w-[45%]">
          <Image
            className="transition duration-300 md:h-full md:group-hover:scale-110"
            objectFit="cover"
            data={{
          ...
          <div className="absolute bottom-0 flex space-x-2 p-3 md:hidden">
            {post.category?.slice(0, 2).map((item: any) => (
              <div key={item.slug} className="bg-white px-1.5 text-sm">
        ...
        <div className="ml-0 flex flex-1 flex-col overflow-auto p-4 transition-all duration-300 md:p-5 md:group-hover:ml-5">
          ...
          <div className="my-3 flex-1 text-sm text-gray-500">
            {post.excerpt}
          </div>
